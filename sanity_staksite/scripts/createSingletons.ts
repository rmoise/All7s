...
console.log('Token present:', !!token) // Will log true/false without exposing the token
console.log('Token length:', token?.length) // Will log the length of the token
...
async function createSingletons() {
  ...
  await testConnection() // Test connection before trying to create documents
