...
export default function MusicAndVideo(videoPreLink) {
    ...
    <div className="text-9xl flex flex-col justify-center items-center">
      {soundCloudMusic.map((album, index) => (
        <div
          ...
          onClick={() => handleClick()}
        >
          <p className="rounded-lg text-2xl w-1/2 mb-3 font-Headline text-white">
            {album.description}
          </p>
  ...
  return (
    <>
      {!mobile ? (
        <video
          className="vid sticky top-0 w-screen h-screen z-10"
          src="https://ik.imagekit.io/a9ltbtydo/stak-images/stak/images/smoke-transition-bg.mp4"
          muted
        ></video>
      ) : (
        <img src="/stak-tape.png" className="mb-12 -z-5" />
      )}
