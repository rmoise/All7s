  ...
  try {
    ...
    for (const album of albums) {
      ...
      try {
        await client
          .patch(album._id)
          .set({ 'customAlbum.songs': updatedSongs })
          .commit();
        console.log(`Successfully updated album: ${album.customAlbum.title || album._id}`);
      } catch (error) {
        console.error(`Failed to update album ${album.customAlbum.title || album._id}:`, error);
        throw error;
      }
    }
